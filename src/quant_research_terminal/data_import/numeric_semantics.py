"""The import layer's view of the shared numeric envelope.

The envelope itself — what counts as a usable price or quantity — is defined
once in :mod:`quant_research_terminal.domain.numeric`. This module holds no
rules of its own; it re-exports the parts the import stages need, so those
stages have one place to import from and no opportunity to restate a rule.
...
Keeping that mapping out of here is what leaves this module a leaf that depends
on nothing above the domain.

Keeping the rules in the domain is what guarantees the property the import
layer exists to uphold: a record that passes validation here constructs as a
domain object, and that object's numeric fields can be encoded exactly by
storage schema v2 without rounding or overflow. Three layers, one definition.

That guarantee covers numeric encoding only. It does not claim the object has
been, or can yet be, written to a file — no Arrow or Parquet IO exists in this
phase.
"""
...
def is_decimal_like(value: object) -> bool:
    ...
    A shape check only — it says nothing about positivity, magnitude, or
    scale. Use :func:`check_price` or :func:`check_quantity` for the full
    envelope.
    """
