...
def _decimal_to_fixed_point(value: Decimal, field_name: str) -> int:
    ...
    Enforces the shared envelope defensively rather than defining a second one:
    a value reaching here should already have satisfied
    ...
    rounded — the envelope has already established that the scaling is exact.

    Raises:
        NumericEnvelopeError: naming the specific violation. Magnitude and
            precision are distinguished, so an out-of-range value is not
...
def _decimal_to_unsigned_int(value: Decimal, field_name: str) -> int:
    ...
    Enforces the shared envelope defensively; see
